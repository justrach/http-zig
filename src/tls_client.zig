//! TLS 1.2/1.3 client copied from Zig std (`std.crypto.tls.Client`) with ALPN
//! `h2` offered on ClientHello. std has no ALPN option; transcript hashing
//! requires the extension to be part of the hello the stack itself sends.
//! Original: Zig contributors, MIT.
...
pub fn init(input: *Reader, output: *Writer, options: Options) InitError!Client {
    ...
    const extensions_payload = tls.extension(.supported_versions, array(u8, tls.ProtocolVersion, .{
        ...
        .psk_dhe_ke,
    })) ++ tls.extension(.application_layer_protocol_negotiation, int(u16, 3) ++ [_]u8{ 2, 'h', '2' }) ++ tls.extension(.key_share, array(
        u16,
        u8,
